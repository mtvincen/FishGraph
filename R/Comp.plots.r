...
{  #split.screen(smatrix)
  ...
  ## kludge for single entry
  if (is.null(dim(m1))){
      irn <- as.integer(yrs.include)
      x1 <- rep(irn,length(m1))
      y1 <- as.numeric(names(m1))
  } else {
  ...
  {
    plot(irn,fit.metric,ylim=c(c.min,1.1),xlim = c(xmin,xmax),type="n",pch=16,ylab="Corr.",las=1, cex.lab=0.9)
    grid(col = "lightgray", lty = 1)
    ...
    points(irn,fit.metric,pch=fit.pch,col=fit.col)
  }
