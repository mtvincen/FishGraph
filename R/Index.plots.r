...
#' @param draw.barplot When \code{TRUE}, a stacked barplot of standardized residuals is drawn.
#' @param draw.pairs When \code{TRUE}, a scatterplot matrix of observed abundance indicies is drawn.
#' 
#' @return Graphics
#' 
#' @author E.H. Williams
...
#' 
#' @examples \donttest{
#' Index.plots.beta(gag)
#' }
#######################################################################################
#  R function to plot abundance indices from fish stock assessment models.
#  Plots observed and predicted and also a stacked barplot of residuals.
#
...
#######################################################################################
{
  #windows(width = 9, height = 11, record = TRUE, xpos = 10, ypos = 10)
...
runs.test.v2=function (y, plot.it = FALSE, alternative = c("two.sided", "positive.correlated", "negative.correlated")) 
{
  ...
  if (plot.it) {
    plot(q, type = "p", pch = "A", col = "red", ylim = c(-0.5, 
                                                         0.5), xlim = c(1, length(y)), xlab = "", ylab = "",main="Runs Test")
...
{   
  ...
  res=res[res.vals]
  yr=ts$year[res.vals]
  index.title=paste("Residual Analysis - Index:", IndexName)
  
  GraphName="residual-analysis-I"
 ...
 { resid.label="Residuals (Ob-Pr)"
  ...
  mtext(index.title,side=3,line=-2,outer=TRUE,cex=1.5)
  ...
  GraphName="residual-analysis-II"
